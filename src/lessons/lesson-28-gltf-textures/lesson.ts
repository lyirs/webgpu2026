...
} from "@/lessons/lesson-28-gltf-textures/glb";
import {
  ...
  normalizeVector,
  type Vector3,
} from "@/lessons/lesson-28-gltf-textures/math";
import fragmentShaderSource from "@/lessons/lesson-28-gltf-textures/model.frag.wgsl?raw";
import vertexShaderSource from "@/lessons/lesson-28-gltf-textures/model.vert.wgsl?raw";
...
type GlbRenderablePrimitive = LoadedTexturedGlbDrawable["primitives"][number] & {
  materialBindGroup: GPUBindGroup;
};

type GlbRenderable = {
  name: string;
  primitives: GlbRenderablePrimitive[];
...
/**
 * 挂载第 18 课“glTF 材质与贴图”预览，并在基础加载上继续接入 `TEXCOORD_0`、图片、纹理和采样器。
 ...
 */
export async function mountGltfTexturesLesson(
  host: HTMLElement,
  ...
  host.innerHTML = `
    <div class="preview-viewport">
      <div class="preview-frame">
        <canvas class="preview-canvas" aria-label="glTF textures lesson preview"></canvas>
      </div>
    </div>
  `;
  ...
  try {
    ...
    /**
     * loadTexturedGlbScene
     * 在基础加载之上继续把 `TEXCOORD_0`、嵌入图片、纹理和 sampler 解析成可直接绘制的场景数据。
     * @param {string} glbUrl lesson 中要加载的模型资源 URL。
     * @param {GPUDevice} device 当前 lesson 使用的 GPUDevice。
     * @returns {Promise<import("./glb").LoadedTexturedGlbScene>} 已经上传到 GPU 的最小带贴图 GLB 场景。
     */
      ...
      vertex: {
        ...
        buffers: [
          ...
          },
          {
            ...
            attributes: [
              {
                // shaderLocation 2：TEXCOORD_0 accessor 解析出来的 2 个 float32。
                shaderLocation: 2,
                offset: 0,
                format: "float32x2",
              },
    ...
    setStatus({
      title: "glTF 材质与贴图已运行",
      detail:
        "这一课已经把 `TEXCOORD_0`、images、textures、samplers 和 `baseColorTexture` 接进 shader。",
      tone: "ok",
    });
