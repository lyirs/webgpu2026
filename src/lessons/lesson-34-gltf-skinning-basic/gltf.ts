import { type Quaternion, type Vector3 } from "@/lessons/lesson-34-gltf-skinning-basic/math";
...
export type LoadedSkinnedGltfPrimitive = {
...
export type LoadedSkinnedGltfNode = {
  name: string;
...
export type LoadedSkinnedGltfScene = {
  meshes: LoadedSkinnedGltfPrimitive[][];
  nodes: LoadedSkinnedGltfNode[];
  rootNodeIds: number[];
  skins: LoadedGltfSkin[];
...
async function loadBufferBytes(
  ...
  const response = await fetch(new URL(buffer.uri, baseUrl).toString());
  if (!response.ok) {
...
/**
 * 从 bufferView 和 accessor 读出一份整数型 joint 数据。
 * @param {Uint8Array[]} buffers 已经加载好的 glTF buffers。
 * @param {GltfBufferViewJson[]} bufferViews glTF 中声明的 bufferView 列表。
 * @param {GltfAccessorJson} accessor 需要读取的 JOINTS_0 accessor。
 * @returns {Uint16Array} 按顶点展开后的 joint 索引数组。
 */
function readJointAccessor(
  buffers: Uint8Array[],
  bufferViews: GltfBufferViewJson[],
  accessor: GltfAccessorJson
): Uint16Array {
  ...
  const values = new Uint16Array(accessor.count * numComponents);

  if (accessor.componentType !== 5121 && accessor.componentType !== 5123) {
    throw new Error("当前 lesson 只支持 uint8 / uint16 类型的 JOINTS_0 accessor。");
  }

  for (let index = 0; index < accessor.count; index += 1) {
    const baseOffset = accessorOffset + index * stride;
    for (let component = 0; component < numComponents; component += 1) {
      values[index * numComponents + component] =
        accessor.componentType === 5121
          ? dataView.getUint8(baseOffset + component)
          : dataView.getUint16(baseOffset + component * scalarByteSize, true);
    }
  }
...
/**
 * 读取一个最小可用的 skinned glTF 场景：POSITION、NORMAL、JOINTS_0、WEIGHTS_0、skin 和 node TRS 动画。
 * @param {string} gltfUrl lesson 中要加载的 glTF 文件 URL。
 * @param {GPUDevice} device 当前 lesson 使用的 GPUDevice。
 * @returns {Promise<LoadedSkinnedGltfScene>} 已上传到 GPU 的网格、节点、skin 和动画数据。
 */
export async function loadSkinnedGltfScene(
  gltfUrl: string,
  device: GPUDevice
): Promise<LoadedSkinnedGltfScene> {
  ...
  const buffers = await Promise.all(
    (json.buffers ?? []).map((buffer) => loadBufferBytes(buffer, baseUrl))
  );
  ...
  const loadedMeshes = meshes.map((mesh, meshIndex) =>
    mesh.primitives.map((primitive, primitiveIndex) => {
      ...
      if (normalAccessorIndex === undefined) {
        throw new Error(`mesh ${meshIndex} primitive ${primitiveIndex} 缺少 NORMAL accessor。`);
      }
      if (jointsAccessorIndex === undefined) {
        throw new Error(`mesh ${meshIndex} primitive ${primitiveIndex} 缺少 JOINTS_0 accessor。`);
      }
      if (weightsAccessorIndex === undefined) {
        throw new Error(`mesh ${meshIndex} primitive ${primitiveIndex} 缺少 WEIGHTS_0 accessor。`);
      }
      ...
      const normalData = readFloat32Accessor(
        buffers,
        bufferViews,
        accessors[normalAccessorIndex]
      );
      const jointsData = readJointAccessor(
        buffers,
        bufferViews,
        accessors[jointsAccessorIndex]
      );
      const weightsData = readFloat32Accessor(
        buffers,
        bufferViews,
        accessors[weightsAccessorIndex]
      );
      ...
      return {
        ...
        jointsBuffer: createGpuBuffer(device, jointsData, GPUBufferUsage.VERTEX),
        weightsBuffer: createGpuBuffer(device, weightsData, GPUBufferUsage.VERTEX),
      ...
      } satisfies LoadedSkinnedGltfPrimitive;
    })
  );
  ...
  const loadedNodes = nodes.map((node, nodeIndex) => {
    ...
    } satisfies LoadedSkinnedGltfNode;
  });

  const loadedSkins = (json.skins ?? []).map((skin, skinIndex) => {
    const inverseBindAccessorIndex = skin.inverseBindMatrices;
    const inverseBindMatrices =
      inverseBindAccessorIndex !== undefined
        ? readFloat32Accessor(
            buffers,
            bufferViews,
            accessors[inverseBindAccessorIndex]
          )
        : new Float32Array(skin.joints.length * 16).map((_, index) =>
            index % 5 === 0 ? 1 : 0
          );

    if (inverseBindMatrices.length !== skin.joints.length * 16) {
      throw new Error(`skin ${skinIndex} 的 inverseBindMatrices 数量和 joints 数量不匹配。`);
    }
