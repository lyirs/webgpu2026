...
fn csSeedScan(@builtin(global_invocation_id) globalId: vec3u) {
  ...
  var previousFlag = 0u;
  if (instanceIndex > 0u) {
    previousFlag = scanFlags[instanceIndex - 1u];
  }
  scanWrite[instanceIndex] = previousFlag;
}
...
fn csPrefixSumStep(@builtin(global_invocation_id) globalId: vec3u) {
  ...
  var previousValue = 0u;
  if (instanceIndex >= scanUniforms.offset) {
    previousValue = scanRead[instanceIndex - scanUniforms.offset];
  }
