...
@group(0) @binding(2) var<uniform> traceParams: TraceParams;

@group(1) @binding(0) var<storage, read> currentPixels: array<vec4f>;
@group(1) @binding(1) var<storage, read_write> accumulatedPixels: array<vec4f>;
@group(1) @binding(2) var<uniform> accumulateParams: AccumulateParams;
...
fn traceScene(origin: vec3f, direction: vec3f) -> HitInfo {
  let boxCount = u32(traceParams.up.w);
...
fn sampleSky(direction: vec3f) -> vec3f {
  ...
  return traceParams.skyColor.xyz * (0.6 + upness * 1.4);
}
...
fn tracePath(origin: vec3f, direction: vec3f, rngState: ptr<function, u32>) -> vec3f {
  ...
  let maxBounce = u32(traceParams.eyeAndBounces.w);
...
fn traceMain(@builtin(global_invocation_id) globalId: vec3u) {
  let width = u32(traceParams.screen.x);
  let height = u32(traceParams.screen.y);
  if (globalId.x >= width || globalId.y >= height) {
    return;
  }

  let pixelIndex = globalId.y * width + globalId.x;
  let seed = u32(traceParams.screen.w);
  let spp = max(u32(traceParams.forwardAndSpp.w), 1u);
  ...
  for (var sampleIndex = 0u; sampleIndex < spp; sampleIndex += 1u) {
    ...
    let rayDirection = normalize(
      traceParams.forwardAndSpp.xyz +
      traceParams.right.xyz * ndc.x +
      traceParams.up.xyz * ndc.y
    );
    radiance += tracePath(traceParams.eyeAndBounces.xyz, rayDirection, &rngState);
  }
...
fn accumulateMain(@builtin(global_invocation_id) globalId: vec3u) {
  let width = u32(accumulateParams.frameInfo.x);
  let height = u32(accumulateParams.frameInfo.y);
  if (globalId.x >= width || globalId.y >= height) {
    return;
  }

  let pixelIndex = globalId.y * width + globalId.x;
  let alpha = accumulateParams.frameInfo.z;
  let currentValue = currentPixels[pixelIndex];
  let previousValue = accumulatedPixels[pixelIndex];
  accumulatedPixels[pixelIndex] = previousValue * (1.0 - alpha) + currentValue * alpha;
}
