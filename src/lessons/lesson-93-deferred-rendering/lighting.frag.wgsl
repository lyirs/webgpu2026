...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  if (worldSample.w > 0.5) {
    ...
    litColor = albedoColor * lightUniforms.ambient.rgb;
    ...
    for (var lightIndex = 0u; lightIndex < 4u; lightIndex += 1u) {
      ...
      let attenuation = 1.0 / (1.0 + distanceSquared * 0.08);
      litColor += albedoColor * lightUniforms.lightColors[lightIndex].rgb * lambert * attenuation;
    }
  }
