...
import { createSpotLightGeometry } from "@/lessons/lesson-09-spot-light/cube-data";
import fragmentShaderSource from "@/lessons/lesson-09-spot-light/spot-light.frag.wgsl?raw";
import {
  ...
  normalizeVector,
  type Vector3,
} from "@/lessons/lesson-09-spot-light/math";
import vertexShaderSource from "@/lessons/lesson-09-spot-light/spot-light.vert.wgsl?raw";
...
/**
 * 把当前帧的投影矩阵、聚光灯方向和光锥参数打包成一块 uniform。
 * @param {Float32Array} viewProjectionMatrix 当前帧的视图投影矩阵。
 * @param {LightState} light 当前聚光灯的位置、方向和锥形范围。
...
function createFrameUniformData(
  ...
  uniformData.set([light.direction[0], light.direction[1], light.direction[2], 0], 20);
  uniformData.set(light.color, 24);
  uniformData.set([light.cone[0], light.cone[1], 0, 0], 28);
  return uniformData;
}
 ...
 * @param {GPURenderPipeline} pipeline 聚光灯渲染管线，用于推导 group(1) 的布局。
 * @param {Float32Array} modelMatrix 当前物体的模型矩阵。
 * @param {[number, number, number, number]} baseColor 当前物体的基础颜色。
...
/**
 * 挂载第 09 课“聚光灯”预览，让点光源继续带上方向和光锥范围。
 ...
 */
export async function mountSpotLightLesson(
  host: HTMLElement,
  ...
  host.innerHTML = `
    <div class="preview-viewport">
      <div class="preview-frame">
        <canvas class="preview-canvas" aria-label="Spot light lesson preview"></canvas>
      </div>
    </div>
  `;
  ...
  try {
    ...
    const geometry = createSpotLightGeometry();
    ...
    /**
     * createBindGroup
     * @param {GPUBindGroupDescriptor} descriptor 绑定组描述对象；这里把聚光灯位置、方向和整帧共享矩阵绑定到 group(0)。
    ...
    const render = (elapsed: number) => {
      ...
      const lightPosition: Vector3 = [
        Math.sin(elapsed * 0.82) * 5.2,
        5.1 + Math.sin(elapsed * 1.3) * 0.35,
        Math.cos(elapsed * 0.82) * 5.2,
      ];
      const lightDirection = normalizeVector([
        cameraTarget[0] - lightPosition[0],
        cameraTarget[1] - lightPosition[1],
        cameraTarget[2] - lightPosition[2],
      ]);
      const light: LightState = {
        position: lightPosition,
        direction: lightDirection,
        color: [2.25, 2.06, 1.62, 1],
        cone: [Math.cos((24 * Math.PI) / 180), Math.cos((36 * Math.PI) / 180)],
      };
       ...
       * @param {AllowSharedBufferSource} data 要拷贝进去的源数据，这里是当前帧的投影矩阵、聚光灯方向和光锥参数。
       ...
       * @param {GPUBindGroup} bindGroup 要绑定的资源组对象；这里包含整帧共享矩阵和聚光灯数据。
    ...
    setStatus({
      title: "聚光灯已运行",
      detail:
        "聚光灯是在点光源基础上再加一层方向和角度限制，所以亮度会更集中地落在一个锥形区域里。",
      tone: "ok",
    });
