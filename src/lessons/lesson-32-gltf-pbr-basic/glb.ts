import {
  ...
  type Quaternion,
  type Vector3,
} from "@/lessons/lesson-32-gltf-pbr-basic/math";
...
type GlbMaterialJson = {
  name?: string;
  normalTexture?: GlbNormalTextureInfoJson;
  pbrMetallicRoughness?: {
    baseColorTexture?: GlbTextureInfoJson;
    baseColorFactor?: number[];
    metallicRoughnessTexture?: GlbTextureInfoJson;
...
export type LoadedPbrGlbMaterial = {
  ...
  metallicRoughnessTextureView: GPUTextureView;
...
export type LoadedPbrGlbPrimitive = {
...
export type LoadedPbrGlbDrawable = {
  name: string;
  baseWorldMatrix: Float32Array;
  primitives: LoadedPbrGlbPrimitive[];
};

export type LoadedPbrGlbScene = {
  drawables: LoadedPbrGlbDrawable[];
  bounds: {
    min: Vector3;
    max: Vector3;
  };
...
function readFloat32Accessor(
  ...
  if (accessor.componentType !== 5126) {
    throw new Error(
      "当前 lesson 只支持 float32 类型的 POSITION / NORMAL / TEXCOORD_0 accessor。"
    );
  }
 ...
 * 创建一张 1x1 的默认纹理。
 * @param {GPUDevice} device 当前 lesson 使用的 GPUDevice。
 * @param {[number, number, number, number]} rgba 纹理像素。
 * @returns {{ texture: GPUTexture; view: GPUTextureView }} 默认纹理及其视图。
 */
...
async function createTextureFromGlbImage(
  ...
  const imageBytes = readBufferViewBytes(
    binaryChunk,
    bufferViews,
    image.bufferView
  );
 ...
 * @param {LoadedPbrGlbPrimitive[][]} meshPrimitives 已经上传到 GPU 的 mesh / primitive 数据。
 * @returns {LoadedPbrGlbDrawable[]} 展开的可绘制对象列表。
 */
function flattenSceneDrawables(
  nodes: GlbNodeJson[],
  rootNodeIds: number[],
  meshPrimitives: LoadedPbrGlbPrimitive[][]
): LoadedPbrGlbDrawable[] {
  const drawables: LoadedPbrGlbDrawable[] = [];
...
/**
 * 读取一个最小可用的 PBR GLB 场景：在 lesson 18 的基础上继续解析 metallic-roughness 和 normal texture。
 * @param {string} glbUrl lesson 中要加载的 GLB 文件 URL。
 * @param {GPUDevice} device 当前 lesson 使用的 GPUDevice。
 * @returns {Promise<LoadedPbrGlbScene>} 已上传到 GPU 的可绘制场景数据。
 */
export async function loadPbrGlbScene(
  glbUrl: string,
  device: GPUDevice
): Promise<LoadedPbrGlbScene> {
  ...
  const defaultWhite = createSolidTexture(device, [255, 255, 255, 255]);
  const defaultMetallicRoughness = createSolidTexture(device, [255, 255, 255, 255]);
  const defaultNormal = createSolidTexture(device, [128, 128, 255, 255]);
  ...
  const defaultMaterial: LoadedPbrGlbMaterial = {
    baseColorTexture: defaultWhite.texture,
    baseColorTextureView: defaultWhite.view,
    baseColorSampler: defaultSampler,
    metallicRoughnessTexture: defaultMetallicRoughness.texture,
    metallicRoughnessTextureView: defaultMetallicRoughness.view,
    normalTexture: defaultNormal.texture,
    normalTextureView: defaultNormal.view,
  ...
  const loadedMaterials = materials.map((material) => {
    ...
    const baseColorGlbTexture =
      baseColorTextureIndex !== undefined ? textures[baseColorTextureIndex] : undefined;
    const metallicRoughnessGlbTexture =
      metallicRoughnessTextureIndex !== undefined
        ? textures[metallicRoughnessTextureIndex]
        : undefined;
    const normalGlbTexture =
      normalTextureIndex !== undefined ? textures[normalTextureIndex] : undefined;

    const baseColorImage =
      baseColorGlbTexture?.source !== undefined
        ? loadedImages[baseColorGlbTexture.source]
        : undefined;
    const metallicRoughnessImage =
      metallicRoughnessGlbTexture?.source !== undefined
        ? loadedImages[metallicRoughnessGlbTexture.source]
        : undefined;
    const normalImage =
      normalGlbTexture?.source !== undefined
        ? loadedImages[normalGlbTexture.source]
        : undefined;

    const samplerIndex =
      baseColorGlbTexture?.sampler ??
      metallicRoughnessGlbTexture?.sampler ??
      normalGlbTexture?.sampler;

    return {
      baseColorTexture: baseColorImage?.texture ?? defaultWhite.texture,
      baseColorTextureView: baseColorImage?.view ?? defaultWhite.view,
      baseColorSampler:
        samplerIndex !== undefined
          ? (loadedSamplers[samplerIndex] ?? defaultSampler)
          : defaultSampler,
      metallicRoughnessTexture:
        metallicRoughnessImage?.texture ?? defaultMetallicRoughness.texture,
      metallicRoughnessTextureView:
        metallicRoughnessImage?.view ?? defaultMetallicRoughness.view,
      normalTexture: normalImage?.texture ?? defaultNormal.texture,
      normalTextureView: normalImage?.view ?? defaultNormal.view,
      baseColorFactor: (pbr?.baseColorFactor ?? [1, 1, 1, 1]) as [
        number,
        number,
        number,
        number,
    ...
    } satisfies LoadedPbrGlbMaterial;
  });
  ...
  const meshPrimitives = meshes.map((mesh, meshIndex) =>
    mesh.primitives.map((primitive, primitiveIndex) => {
      ...
      if (positionAccessorIndex === undefined) {
        throw new Error(
          `mesh ${meshIndex} primitive ${primitiveIndex} 缺少 POSITION accessor。`
        );
      }
      ...
      return {
        ...
        material:
          primitive.material !== undefined
            ...
            : defaultMaterial,
      } satisfies LoadedPbrGlbPrimitive;
    })
  );
