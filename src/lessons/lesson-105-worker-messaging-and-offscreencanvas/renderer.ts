import {
  ...
  type WorkerMessagingLessonGeometry,
} from "@/lessons/lesson-105-worker-messaging-and-offscreencanvas/geometry";
import {
...
import fragmentShaderSource from "@/lessons/lesson-105-worker-messaging-and-offscreencanvas/scene.frag.wgsl?raw";
import vertexShaderSource from "@/lessons/lesson-105-worker-messaging-and-offscreencanvas/scene.vert.wgsl?raw";
...
export type WorkerMessagingRenderer = {
 ...
 * @returns {Promise<WorkerMessagingRenderer>} 一个可以 resize / render / destroy 的运行时对象。
 */
export async function createThreadRenderer(
  canvas: HTMLCanvasElement | OffscreenCanvas,
  initialDimensions: CanvasDimensions
): Promise<WorkerMessagingRenderer> {
  if (!("gpu" in navigator)) {
  ...
  const geometry: WorkerMessagingLessonGeometry = createWorkerMessagingLessonGeometry();
  const sceneConfigs = createWorkerMessagingSceneConfigs();
