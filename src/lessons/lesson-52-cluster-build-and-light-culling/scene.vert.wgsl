...
@vertex
fn vsObject(input: ObjectVertexInput) -> ObjectVertexOutput {
  var output: ObjectVertexOutput;
  let worldPosition = objectUniforms.modelMatrix * vec4f(input.position, 1.0);
  let worldNormal = transformStaticNormal(objectUniforms.modelMatrix, input.normal);

  output.clipPosition = sceneUniforms.viewProjectionMatrix * worldPosition;
  output.worldPosition = worldPosition.xyz;
  output.worldNormal = worldNormal;
  output.baseColor = objectUniforms.color.xyz;
  return output;
}
...
fn vsLightMarker(
  ...
  output.clipPosition = sceneUniforms.viewProjectionMatrix * worldPosition;
  output.color = light.colorIntensity.rgb * (1.08 + light.colorIntensity.a * 0.08);
  return output;
}
