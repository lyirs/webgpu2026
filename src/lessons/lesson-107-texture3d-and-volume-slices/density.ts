export type Texture3dSliceDensityData = {
  size: number;
  data: Uint8Array;
  voxelCount: number;
  memoryBytes: number;
...
/**
 * 生成一份单通道 3D 体密度数据，后续会直接上传到 `texture3D`。
 * @param {number} size 体纹理边长，默认 64。
 * @returns {Texture3dSliceDensityData} 对应的体素字节数据和摘要信息。
 */
export function createTexture3dSliceDensityTextureData(
  size = 64
): Texture3dSliceDensityData {
  ...
  for (let z = 0; z < size; z += 1) {
    ...
    for (let y = 0; y < size; y += 1) {
      ...
      for (let x = 0; x < size; x += 1) {
        ...
        const shell = Math.max(0, 1 - Math.abs(radius - 0.68) * 5.2) * 0.12;
        ...
        let density = (core + plumeOne + plumeTwo + ribbon + shell + wave) * falloff;
