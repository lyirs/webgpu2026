...
@group(0) @binding(6) var<storage, read_write> currentValues: array<vec4f>;
...
fn occluderCount() -> u32 { return u32(settings.historyInfo.x); }
fn lightAnimationEnabled() -> bool { return settings.historyInfo.w > 0.5; }
...
fn animatedLightPosition(light: LightData, lightIndex: u32) -> vec2f {
  if (!lightAnimationEnabled()) {
    return light.positionRadius.xy;
  }
  let fx = sin(frameTime() * 0.0007 + f32(lightIndex) * 0.61) * 0.018;
  let fy = cos(frameTime() * 0.0005 + f32(lightIndex) * 0.41) * 0.01;
  return light.positionRadius.xy + vec2f(fx, fy);
}
...
fn evaluateLightContribution(world: vec2f, owner: f32, modulation: f32, lightIndex: u32) -> f32 {
  ...
  let lightPosition = animatedLightPosition(light, lightIndex);
...
fn buildSpatialMain(@builtin(global_invocation_id) globalId: vec3u) {
  ...
  currentValues[index] = vec4f(estimateReservoirLighting(finalReservoir), 0.0, 0.0, 0.0);
}
