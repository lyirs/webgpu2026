...
/**
 * 基于世界空间坐标做一层细网格，让分辨率差异更容易被看见。
 */
fn gridMask(coord: vec2f, scale: f32) -> f32 {
  let scaled = coord * scale;
  let lineDistance = abs(fract(scaled - 0.5) - 0.5) /
    max(fwidth(scaled), vec2f(0.0001));
...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  let attenuation =
    1.0 / (1.0 + lightDistance * 0.17 + lightDistance * lightDistance * 0.028);
  ...
  if (input.surfaceParams.x > 0.5) {
    let darkened = surfaceColor * vec3f(0.48, 0.50, 0.55);
    let highlighted =
      min(surfaceColor * vec3f(1.16, 1.2, 1.26) + vec3f(0.05, 0.07, 0.09), vec3f(1.0));
  ...
  let specular =
    ...
    attenuation *
    0.38;
  let rim = pow(1.0 - max(dot(normal, viewDirection), 0.0), 2.8) * 0.12;
