...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  let sunRadiance = vec3f(22.0, 17.5, 13.2) * frameUniforms.sceneParams.x;
  ...
  let directColor = (directDiffuse + directSpecular) * sunRadiance * normalDotLight;

  let diffuseEnvironment = srgbToLinear(
    textureSample(environmentTexture, environmentSampler, normal).rgb
  );
  ...
  let iblColor =
    (iblDiffuse + iblSpecular) * frameUniforms.sceneParams.y;

  let ambientFloor = albedo * vec3f(0.012, 0.01, 0.014);
  let color = ambientFloor + directColor + iblColor;

  return vec4f(color, baseColorSample.a);
}
