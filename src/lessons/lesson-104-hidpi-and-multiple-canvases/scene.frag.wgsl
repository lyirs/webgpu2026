...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  if (input.surfaceParams.x > 0.5) {
    ...
    let highlighted = min(surfaceColor * vec3f(1.16, 1.2, 1.26) + vec3f(0.05, 0.07, 0.09), vec3f(1.0));
  ...
  let specular = vec3f(1.0, 0.98, 0.94) * pow(max(dot(normal, halfVector), 0.0), 28.0) * attenuation * 0.38;
