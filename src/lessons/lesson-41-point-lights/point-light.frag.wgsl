...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  let attenuation = 1.0 / (1.0 + 0.12 * lightDistance + 0.04 * lightDistance * lightDistance);
  let lambert = max(dot(normal, lightDirection), 0.0);
  let ambient = vec3f(0.15, 0.16, 0.2);
  let diffuse = frameUniforms.lightColor.rgb * lambert * attenuation;
