...
fn fsMain(input: FragmentInput) -> @location(0) vec4f {
  ...
  let radiance = vec3f(4.8, 4.5, 4.2);
  ...
  let specular = specularNumerator / max(vec3f(denominator), vec3f(0.001));
  ...
  let color = ambient + (diffuse + specular) * radiance * normalDotLight;
