 ...
 * @returns {Float32Array} `left * right` 的结果矩阵。
 */
...
/**
 * 创建 Reversed-Z 透视投影矩阵：近处映射到 1，远处映射到 0。
 * @param {number} fieldOfViewRad 垂直视场角，单位为弧度。
 * @param {number} aspect 画布宽高比。
 * @param {number} near 近裁剪面距离。
 * @param {number} far 远裁剪面距离。
 * @returns {Float32Array} 对应的 Reversed-Z 投影矩阵。
 */
export function createReversedZPerspectiveMatrix(
  fieldOfViewRad: number,
  aspect: number,
  near: number,
  far: number
 ...
 * @returns {Vector3} 单位化结果；零向量会回退成 [0, 0, 0]。
 */
 ...
 * @param {Vector3} up 用于约束头顶方向的参考向量。
