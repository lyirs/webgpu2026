...
import fragmentShaderSource from "@/lessons/lesson-44-free-orbit-camera/cube.frag.wgsl?raw";
import { createFreeOrbitCubeGeometry } from "@/lessons/lesson-44-free-orbit-camera/cube-data";
import {
  addVectors,
  ...
  scaleVector,
  type Vector3,
} from "@/lessons/lesson-44-free-orbit-camera/math";
import vertexShaderSource from "@/lessons/lesson-44-free-orbit-camera/cube.vert.wgsl?raw";
...
/**
 * 挂载第 09 课“自由轨道相机”预览，并完成无俯仰限制的相机旋转。
 ...
 */
export async function mountFreeOrbitCameraLesson(
  host: HTMLElement,
  ...
  host.innerHTML = `
    <div class="preview-viewport">
      <div class="preview-frame">
        <canvas class="preview-canvas" aria-label="Free orbit camera lesson preview"></canvas>
      </div>
    </div>
  `;
  ...
  try {
    ...
    const cube = createFreeOrbitCubeGeometry();
    ...
    const pipeline = gpu.device.createRenderPipeline({
      label: "lesson-44-free-orbit-camera",
      layout: "auto",
      vertex: {
        ...
        buffers: [
          {
            // 顶点顺序仍然是 [position.xyz, color.rgb, normal.xyz]，这样我们可以只聚焦相机变化。
            arrayStride: 9 * 4,
            attributes: [
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      // 水平拖拽时围绕当前 up 旋转；因为 up 本身也会跟着转，所以这里不会被锁死在世界 y 轴上。
      ...
      // 垂直拖拽时围绕当前 right 旋转；这一步不再做 pitch clamp，所以可以自由越过顶部。
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      basis = {
        ...basis,
        radius: clamp(basis.radius + event.deltaY * 0.01, 3.2, 9),
      };
      render();
    };
    ...
    setStatus({
      title: "自由轨道相机已接入",
      detail:
        "拖拽现在不再依赖 pitch 安全角，视角可以越过顶部继续旋转；滚轮仍然控制观察半径。",
      tone: "ok",
    });
